...
export default {
  ...
  methods: {
    ...
    getTimeStatus: function (time, no, format) {
      ...
      let selTime = this['time' + no]
      let tmpTimeVal = this.stringify(time, format || 'yyyy-MM-dd') // 需要查询状态的时间字符串值
      let curTimeVal = this.stringify(curTime, format || 'yyyy-MM-dd') // 当前时间字符串值
      let selTimeVal = this.stringify(selTime, format || 'yyyy-MM-dd') // 选中时间字符串值
    ...
    showYear: function (no) {
      ...
      this.hidePanel(name)
      this[name] = !this[name]
      let time = new Date(this['now' + no] || new Date())
      let num = Math.floor(time.getFullYear() % 10) // 获取当前时间个位数
      let arr = []
    ...
    changeYearRange: function (no, flag) {
      let arr = this['years' + no]
      let time = new Date(this['time' + no] || new Date())
      for (var i in arr) {
        let item = arr[i]
        let year = item.year + 10 * flag
        time.setDate(1) // 先设置为第一天，因为月份天数不一样，要不存在bug
